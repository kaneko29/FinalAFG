...
func _ready():
	...
	quitButton.modulate = Color(1, 0, 0, 1)
	...
	checkButton.rect_position = Vector2(3 * screenx/4 - checkButton.rect_size.x * buttonscale.x / 2, ytransl/2 - checkButton.rect_size.y * buttonscale.y / 2)
	quitButton.rect_position = 	Vector2(screenx/4 - quitButton.rect_size.x * buttonscale.x / 2, ytransl/2 - quitButton.rect_size.y * buttonscale.y / 2) 
...
func anscheck():
	...
	if counter == N:
		quitButton.modulate = Color(0, 1, 0, 1)
